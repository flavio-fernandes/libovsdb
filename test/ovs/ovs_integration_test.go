...
func (suite *OVSIntegrationSuite) TestSuryaTwoTxns() {
	...
	require.NoError(suite.T(), err)
	operationErrs, err := ovsdb.CheckOperationResults(reply, ops)
	if err != nil {
	...
	br_uuid := reply[0].UUID.GoUUID
	br.UUID = br_uuid
	require.Eventually(suite.T(), func() bool {
		bridgeRow := &bridgeType{UUID: br_uuid}
		err = suite.client.Get(context.Background(), bridgeRow)
		if err != nil {
			return false
		}
		// return reflect.DeepEqual(br, bridgeRow)
		return bridgeRow.Name == bridgeName
	}, 2*time.Second, 50*time.Millisecond)
	...
	delMutateOp, err := suite.client.WhereCache(func(*ovsType) bool { return true }).
		Mutate(&ovsRow, model.Mutation{
			...
			Mutator: ovsdb.MutateOperationDelete,
			// Value:   []string{namedUUID},
			Value:   []string{br_uuid},
		})
	require.NoError(suite.T(), err)
	ops = append(ops, delMutateOp...)

	br = bridgeType{
		UUID: br_uuid,
	}
	deleteOp, err := suite.client.Where(&br).Delete()
	require.NoError(suite.T(), err)
	ops = append(ops, deleteOp...)

	reply2, err := suite.client.Transact(context.TODO(), ops...)
	require.NoError(suite.T(), err)

	operationErrs2, err := ovsdb.CheckOperationResults(reply2, ops)
	if err != nil {
		for _, oe := range operationErrs2 {
			suite.T().Error(oe)
		}
		suite.T().Fatal(err)
	}
...
func (suite *OVSIntegrationSuite) TestSuryaSingleTxns() {
